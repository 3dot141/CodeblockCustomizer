...
function createDecorationWidget(textToDisplay, displayLanguageName, languageName, specificHeader, defaultFold2, hasLangBorderColor, settings, marginLeft) {
  return import_view.Decoration.widget({ widget: new TextAboveCodeblockWidget(textToDisplay, displayLanguageName, languageName, specificHeader, defaultFold2, hasLangBorderColor, settings, marginLeft), block: true });
}
...
var TextAboveCodeblockWidget = class extends import_view.WidgetType {
  constructor(text, displayLanguageName, languageName, specificHeader, defaultFold2, hasLangBorderColor, settings, marginLeft) {
    super();
  ...
  eq(other) {
    return other.text === this.text && other.displayLanguageName === this.displayLanguageName && other.languageName === this.languageName && other.specificHeader === this.specificHeader && other.defaultFold === this.defaultFold && other.hasLangBorderColor === this.hasLangBorderColor && other.enableLinks === this.enableLinks && other.marginLeft === this.marginLeft;
  }
  toDOM(view) {
    this.view = view;
          ...
          (0, import_language.syntaxTree)(view.state).iterate({
            ...
            enter(node) {
              ...
              if (node.type.name === "HyperMD-codeblock_HyperMD-codeblock-bg" || startLine || endLine) {
                ...
                if (indentLevel > 0) {
                  decorations.push(import_view2.Decoration.mark({ class: "codeblock-customizer-hidden-element" }).range(node.from, node.from + indentChars));
                  decorations.push(import_view2.Decoration.line({ attributes: { "style": `margin-left:${marginLeft}px !important` } }).range(node.from));
                }
                lineNumber++;
              }
            }
...
function processLineText(lineText, codeblockId, alternateColors) {
  ...
  if (lineText) {
    ...
    const regex = /\{(.*)\}/g;
    const execArray = regex.exec(lineText);
